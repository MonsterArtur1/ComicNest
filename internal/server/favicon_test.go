...
func TestFaviconServedWithoutLogin(t *testing.T) {
	...
	for _, path := range []string{"/favicon.ico", "/static/favicon.svg", "/static/favicon.png", "/static/apple-touch-icon.png"} {
	...
	for _, path := range []string{"/", "/login", "/issues/1/read"} {
		...
		if !strings.Contains(body, `<link rel="icon" href="/static/favicon.svg"`) {
