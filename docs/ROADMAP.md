...
- [x] Multiple independent libraries: an optional `libraries: []string` list in `config.yaml`, taking precedence over the single `library` key (unset/empty `libraries` leaves existing single-folder configs unchanged); a `library.Scanner` per root (`Scanner.underRoot` keeps one library's scan from touching another's catalog); `series`/`scan_history` stamped with their library (migration 12); the admin panel splits Statistics/Scan/History into one block per library (scan button above its stats) plus a "Scan All Libraries" button once more than one is configured; a main-menu library switcher (`POST /library`, `comicnest_library` cookie) scopes the home grid and search. A library removed from `config.yaml` has its issues flagged missing (`Store.MarkOrphanedLibrariesMissing`, run at startup and on every scan) since no scanner will ever reach them again. A `library_names` database table (migration 13) lets the admin panel rename a library's display name in place (`POST /admin/libraries/{index}/name`, `Store.SetLibraryName`, applies immediately, no restart). OPDS keeps serving one combined catalog regardless, by design (SPECIFICATION §4, §5, §6, §9, §9a)
- [x] A "Your stats" page (`GET /stats`, linked from the stats strip on the library page as "all stats →"): all-time counts from `Store.UserStats` (issues read, pages read, in progress, series started/completed, favorite issues), activity from the finish timestamps (`computeReadingStats`: this week (Monday-first)/month/year, current/longest streak, days with a finish), a 12-month and a by-weekday bar chart (plain CSS, `statBar.Pct`), and top-5 rankings of the most read series, publishers (issue's, falling back to the series') and writers (credits split on commas, merged case-insensitively). Per user, across every library, present-on-disk issues only
