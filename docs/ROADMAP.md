...
- [x] Docker: `Dockerfile` (static binary w `distroless/static:nonroot`, ok. 15 MB), `docker-compose.yml`, `.dockerignore`; wolumeny `/comics` (ro) `/config` `/data`; obraz w GHCR (`ghcr.io/monsterartur1/comicnest`, amd64+arm64) z joba `docker` w workflow. W aplikacji: flaga `-config` / `$COMICNEST_CONFIG`, nadpisania `COMICNEST_*` (`Config.applyEnv`), `GET /healthz` i tryb `-healthcheck` do `HEALTHCHECK`
