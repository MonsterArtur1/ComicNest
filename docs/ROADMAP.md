...
- Admin panel: edit `config.yaml` values (ComicVine key, `page_size`, `listen`, `opds_enabled`) from the browser instead of the file — some settings would still need a restart to take effect
- Admin panel: a "test connection" button for the ComicVine API key, instead of only finding out it's bad at scrape time
- Admin panel: a log/history of past scans (when, how many files, errors) instead of just the current status
